...
class OpenAIClient:
    ...
    def _get_structured_output(
        ...
        results = self.client.responses.parse(
            model=self.model,
            input=[
    ...
    def upload_file(
        self, file: str | BinaryIO | Path, purpose: FilePurpose = FilePurpose.USER_DATA
    ) -> FileObject:
        """Upload"""
    ...
    def parse_user_file(
        ...
        user_prompt = {
            "role": "user",
            "content": [{"type": "input_file", "file_id": file_result.id}],
        }
